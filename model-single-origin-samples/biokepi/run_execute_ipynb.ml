...
let ii_pip_requirements_path = "/tmp/dummy-requirements.txt"
...
let ii_witness = install_path // ".ii"
let ii_node =
  workflow_node (single_file ~host ii_witness)
    ...
    ~make:(run_program
             Program.(
               conda_init &&
               shf "pip install -r %s" ii_pip_requirements_path &&
...
let submit_job
  ...
  let master_node =
    workflow_node without_product
      ...
      ~make:(run_program
               Program.(
                 shf "jupyter nbconvert --to notebook %s --output %s --ExecutePreprocessor.timeout=%d" my_notebook my_notebook timeout
               )
            )
  in
...
let my_notebook =
  let doc = "Jupyter notebook containing data-loading and trains a stan model" in
...
let cmd =
  let doc = "Execute jupyter notebook in-place." in
